...
) ENGINE=InnoDB AUTO_INCREMENT=4353 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each person who is associated with a board.  This table controls the permissions the individual has on this board.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=16315 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=3054 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each status (i.e. column) that appears on a board.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=7929 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each person who may use or be referred to by the system.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=4017 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each project in the system.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_stakeholder` (
  ...
  KEY `person_id_idx` (`person_id`)
) ENGINE=InnoDB AUTO_INCREMENT=204 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each individual who has a stake in the outcome of this project.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `property` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1604 DEFAULT CHARSET=utf8 COMMENT='Stores a property associated with a given object';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sprint` (
  ...
  PRIMARY KEY (`id`),
  KEY `board_id_idx` (`board_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1271 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each sprint for a given board.  Note that there is always a virtual sprint "0" for any board that represents a custom date range for filtering, but that may not be assigned to a task.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `tag` (
  ...
  PRIMARY KEY (`id`),
  KEY `tag_idx` (`tag`)
) ENGINE=InnoDB AUTO_INCREMENT=613 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each unique tag in the system';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=13279 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each task that may be associated with a project.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `task_approver` (
  ...
  KEY `task_id_idx` (`task_id`),
  KEY `person_id_idx` (`person_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3389 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each approver associated with a task';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=4954 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each assignee associated with a task';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=502 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each comment associated with a task';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=2714 DEFAULT CHARSET=utf8 COMMENT='Contains one row for each tag associated with a task';
/*!40101 SET character_set_client = @saved_cs_client */;
...
*/
	...
	call sp_get_task(LAST_INSERT_ID());
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_get_all_board_data` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`mykanban`@`%` PROCEDURE `sp_get_all_board_data`(
	b_board_id bigint,
	b_sprint_id bigint,
	b_project_id bigint,
	b_assignee_id bigint,
	b_approver_id bigint,
	b_search varchar(100),
	b_user_id bigint
)
BEGIN

	-- Board
	DROP TEMPORARY TABLE IF EXISTS _board;
	CREATE TEMPORARY TABLE _board AS  
		select * from board 
			where id = b_board_id;

	-- Sprints
	DROP TEMPORARY TABLE IF EXISTS _sprint;
	CREATE TEMPORARY TABLE _sprint AS  
		select *, concat(sequence, ': ', start_date, ' - ', end_date) as name from sprint 
			where board_id = b_board_id;

	-- Project
	DROP TEMPORARY TABLE IF EXISTS _project;
	CREATE TEMPORARY TABLE _project AS  
	select project.* from project, board_project 
		where project.id = board_project.project_id
		  and board_project.board_id = b_board_id
		order by name;

	-- Tasks
	DROP TEMPORARY TABLE IF EXISTS _task;
	CREATE TEMPORARY TABLE _task AS  
	select task.*, board_set_status.fore_color, board_set_status.back_color, 
			project.name as project_name 
		from task, board_set_status, project
		where project_id IN (select id from _project)
		  and task.status_id = board_set_status.id
		  and task.project_id = project.id
		  and (task.project_id = b_project_id OR b_project_id = 0)
		  and (task.name LIKE concat('%',b_search,'%') OR task.define_done LIKE concat('%',b_search,'%'))
		  and (task.id IN (select task_id from task_assignee WHERE (task_assignee.person_id = b_assignee_id)) OR b_assignee_id = 0)
		  and (task.id IN (select task_id from task_approver WHERE (task_approver.person_id = b_approver_id)) OR b_approver_id = 0)
		  and (task.id IN (select t2.id from task t2, sprint WHERE (t2.start_date <= sprint.end_date AND t2.end_date >= sprint.start_date AND sprint.id = b_sprint_id) OR b_sprint_id = 0))
		order by task.sequence;

	-- Assignees
	DROP TEMPORARY TABLE IF EXISTS _task_assignee;
	CREATE TEMPORARY TABLE _task_assignee AS  
		select person.*, task_assignee.task_id from person, task_assignee 
			where person.id = task_assignee.person_id 
			  AND task_assignee.task_id IN (select id from _task);

	-- Approvers
	DROP TEMPORARY TABLE IF EXISTS _task_approver;
	CREATE TEMPORARY TABLE _task_approver AS  
		select person.*, task_approver.task_id  from person, task_approver 
			where person.id = task_approver.person_id 
			  AND task_approver.task_id IN (select id from _task);

	-- Users
	DROP TEMPORARY TABLE IF EXISTS _board_person;
	CREATE TEMPORARY TABLE _board_person AS  
		select person.*, board_person.can_add, board_person.can_delete, board_person.can_edit, board_person.can_read 
			from person, board_person 
			where person.id = board_person.person_id 
			  AND board_person.board_id = b_board_id
			order by person.name;

	-- Status Codes
	DROP TEMPORARY TABLE IF EXISTS _board_set_status;
	CREATE TEMPORARY TABLE _board_set_status AS  
	select * from board_set_status 
		where board_set_id = (select board_set_id from _board)
		order by sequence;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_get_person_by_user_name` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`mgerow`@`%` PROCEDURE `sp_get_person_by_user_name`(
	p_user_name varchar(100)
)
BEGIN
/* 	
	---------------------------------------------------------------------------
	Purpose:	Retrieve all data for a specific individual by user name

	Params:		p_user_name .... user_name of person to retrieve
	---------------------------------------------------------------------------
	Mods:
	---------------------------------------------------------------------------
*/
...
*/
	...
	FROM 	task t LEFT OUTER JOIN project p ON t.project_id = p.id 
				LEFT OUTER JOIN board_project bp ON p.id = bp.project_id
				LEFT OUTER JOIN board b ON bp.board_id = b.id
				LEFT OUTER JOIN board_set bs ON b.board_set_id = bs.id
				LEFT OUTER JOIN board_set_status bss ON t.status_id = bss.id

	WHERE 	t.id = t_task_id;
	...
	-- CREATE TEMPORARY TABLE _task_assignee AS  
		select person.*, task_assignee.task_id from person, task_assignee 
			where person.id = task_assignee.person_id 
			  AND task_assignee.task_id = t_task_id;
	...
	-- CREATE TEMPORARY TABLE _task_approver AS  
		select person.*, task_approver.task_id  from person, task_approver 
			where person.id = task_approver.person_id 
			  AND task_approver.task_id = t_task_id;
...
-- Dump completed on 2015-03-31 21:58:02
